...
async def choix_voyage_et_question(state: State) -> dict[str, Any]:
    ...
    # Lors du premier tour de la conversation, aucun critère n'est encore défini
    # (en général l'utilisateur dit juste bonjour)
    # NOTE: le graphe est rejoué à chaque message utilisateur,
    # voir la version "interrupt" pour une alternative en un seul tour
    if not state.au_moins_un_critere_rempli():
        prompt_reponse += f"""
        ...
        """
    # Lors du second tour de la conversation, l'utilisateur a normalement répondu à la question
    # en fournissant des critères.
    else:
        prompt_reponse += f"""
...
graph = (
    ...
    .compile(name="Agent de voyage (basique en multi-tours)")
)
